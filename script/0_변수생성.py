...
for year in [2016,2017,2019,2020,2021]:
    ...
    for fn in data_files:
        ...
        df_tp['remove']=df_tp['remove'].interpolate(method='pad')#장기 결측
        ...
        df_tp=df_tp[df_tp['times'].dt.minute==0]# 1시간 간격자료 추출
        df_tp=lonlatProj.create_vars(df_tp)# 변수 생성
        df_tp=df_tp.drop(['lat_rad','lon_rad','p_lat_rad','p_lon_rad'],axis=1)#불필요 변수 제거
        if year==2021:
            df_tp.to_csv(rf'..\output\before_qc/test/{fn}',index=False,encoding='cp949')
        else:
            df_tp.to_csv(rf'..\output\before_qc/train/{fn}',index=False,encoding='cp949')
